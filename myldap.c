...
int anonymous_user_search(char *username)
{
    ...
    fprintf(stdout, "connected to LDAP server %s\n", LDAP_URI);

    if((rc = ldap_set_option(ld, LDAP_OPT_PROTOCOL_VERSION, &ldapversion)) !=
       LDAP_SUCCESS) // protokoll versionsnr muss stimmen sonst
    {
    ...
    snprintf(filter, MAX_FILTER_LEN, "(uid=%s)", username);
    // perform ldap search
...
{
    int status = 0;
    status = anonymous_user_search(username);
